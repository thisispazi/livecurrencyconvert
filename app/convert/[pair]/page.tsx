...
<section style={{ background: '#1e40af', color: 'white', padding: '48px 0' }}>
  <div className="container" style={{ textAlign: 'center' }}>
    ...
    <h1 style={{ fontSize: '42px', fontWeight: '800', marginBottom: '32px' }}>
      {from} to {to} Exchange Rate
    </h1>
    {rate ? (
      <Calculator rate={rate} from={from} to={to} />
    ) : (
            ...
            <div style={{ maxWidth: '600px', margin: '0 auto', background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
              ...
              {AMOUNTS.map((amount, i) => (
                <div key={amount} style={{
                  ...
                  borderTop: '1px solid #f1f5f9', fontSize: '15px'
                }}>
                  <span style={{ fontWeight: '600', color: '#1e293b' }}>{amount.toLocaleString()} {from}</span>
          ...
          {[
            {
              ...
              a: rate ? `1 ${fromCurrency.name} (${from}) is currently equal to ${rate.toFixed(4)} ${toCurrency.name} (${to}). This rate is updated every hour.` : `Please check the live rate above.`
            },
            {
        ...
        <div className="container">
          ...
          <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center' }}>
            {Object.keys(CURRENCIES).filter(c => c !== from && c !== to).slice(0, 8).map(c => (
              <Link key={c} href={`/convert/${from.toLowerCase()}-to-${c.toLowerCase()}`}
                style={{ padding: '10px 18px', background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '8px', textDecoration: 'none', color: '#1e40af', fontWeight: '600', fontSize: '14px' }}>
                {from} → {c}
              </Link>
            ))}
          </div>
